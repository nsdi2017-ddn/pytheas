...
if __name__ == '__main__':
    ...
    newoutput = 'result-detail.txt'
    foutnew = open(newoutput, 'w')
    foutnew.write("")
    foutnew.close()
    ...
    for trace in trace_list:
        ...
        if int(time.time() - test_start_time) > test_second:
            ...
            send_num = 0
            foutnew = open(newoutput, 'a')
            foutnew.write(str(cost_list[test_second-1] / request_num[test_second-1][1]) + '\n')
            foutnew.close()
            fout.write(str(cost_list[test_second-1] / request_num[test_second-1][1]) + '\n')
            print "------------------------------ %3d sec" % test_second
            #print request_num
            #print load_dict_list
            #print cost_list
